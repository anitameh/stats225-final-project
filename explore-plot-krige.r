...
# read in mobility data (data does NOT include AK and HI) & long-lat data
...
print(summary(abs_mobility_std))
new_abs = na.omit(abs_mobility_std)
print(sd(new_abs))
...
# create geodata for nonstandardized data (for comparison purposes)
abs_notstd_newdata = cbind(abs_mobility, jittered_coords)
my_abs_notstd_geodata = as.geodata(abs_notstd_newdata, coords.col = 2:3, data.col = 1)

# plot variog cloud using robust estimator
my_abs_notstd_variog = variog(my_abs_notstd_geodata, 
                              option="cloud", estimator.type="modulus")
png( "absolute_mobility_notstd_cloud_unrestricted.png", 
     width = 5000, height = 4500 )
plot(my_abs_notstd_variog$u, my_abs_notstd_variog$v, pch=10,
     col=rgb(red=0, green=255, blue=0, alpha=50, max=255),
     xlab="Distance", ylab="Semivariance",
     main = "Cressie estimator, Distance Unrestricted")
dev.off()

my_abs_notstd_variog = variog(my_abs_notstd_geodata, option="cloud", 
                       max.dist=20, estimator.type="modulus")
png( "absolute_mobility_notstd_cloud_restricted.png", width = 5000, height = 4500 )
plot(my_abs_notstd_variog$u, my_abs_notstd_variog$v, pch=10,
     col=rgb(red=0, green=255, blue=0, alpha=50, max=255),
     xlab="Distance", ylab="Semivariance",
...
# predict
abs_krige_pred = krige.conv(my_abs_geodata, locations=pred_grid, 
                            krige=abs_krige_control)
...
# regress on a matrix that has dim [number of counties x 48] 
#   each element is 1 if the measurement is in that state and 0 o/w
state_effect = read.csv(paste(wd, "data/all-ones.csv", sep=""), header=FALSE)
linear_reg = lm(aa ~ state_effect[,1] + state_effect[,2] + state_effect[,3] +
                  state_effect[,4] + state_effect[,5] + state_effect[,6] + 
                  state_effect[,7] + state_effect[,8] + state_effect[,9] +
                  state_effect[,10] + state_effect[,11] + state_effect[,12] +
                  state_effect[,13] + state_effect[,14] + state_effect[,15] +
                  state_effect[,16] + state_effect[,17] + state_effect[,18] + 
                  state_effect[,19] + state_effect[,20] + state_effect[,21] +
                  state_effect[,22] + state_effect[,23] + state_effect[,24] +
                  state_effect[,25] + state_effect[,26] + state_effect[,27] +
                  state_effect[,28] + state_effect[,29] + state_effect[,30] + 
                  state_effect[,31] + state_effect[,32] + state_effect[,33] +
                  state_effect[,34] + state_effect[,35] + state_effect[,36] +
                  state_effect[,37] + state_effect[,38] + state_effect[,39] +
                  state_effect[,40] + state_effect[,41] + state_effect[,42] + 
                  state_effect[,43] + state_effect[,44] + state_effect[,45] +
                  state_effect[,46] + state_effect[,47] + state_effect[,48])
residuals = linear_reg$residuals
# conduct kriging on residuals